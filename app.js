// app.js
// Versión reescrita con conteo de docentes únicos por colegio
// y avance del ciclo basado en docente-talleres únicos.
...
// Lee un campo qN respetando valores 0.
...
// Formatea un valor de fecha a "DD/MM/YYYY" para mostrar.
...
// Devuelve null si no se puede interpretar de forma fiable.
...
// -----------------------------------------------------------------------------
// Conteo confiable de docentes únicos
// -----------------------------------------------------------------------------
...
function normalizarNombrePersona(nombre) {
    let clean = normalizarTextoBase(nombre);
    ...
    if (invalidos.has(clean)) return '';

    clean = clean
        .replace(/\b(sr|sra|senor|senora|señor|señora|prof|profe|lic|mg|magister|esp|docente)\b/g, '')
        .replace(/\s+/g, ' ')
        .trim();
...
function normalizarColegioKey(colegio) {
    return normalizarTextoBase(colegio).toUpperCase();
}
...
function perfilCoincide(d, perfilObjetivo) {
    return normalizarTextoBase(d.perfil) === normalizarTextoBase(perfilObjetivo);
}

// Cuenta personas únicas dentro de cada colegio.
// La clave es colegio + nombre_normalizado.
function contarPersonasUnicasPorColegio(encuestas, perfilObjetivo = 'Docente') {
    const clavesGlobales = new Set();
    const porColegio = {};
    let descartadosSinNombre = 0;
    let registrosValidos = 0;

    encuestas.forEach(d => {
        if (!perfilCoincide(d, perfilObjetivo)) return;

        const colegioKey = normalizarColegioKey(d.colegio);
        const personaKey = normalizarNombrePersona(d.asistente);

        if (!colegioKey || !personaKey) {
            descartadosSinNombre++;
            return;
        }

        registrosValidos++;

        const clave = `${colegioKey}||${personaKey}`;
        clavesGlobales.add(clave);

        if (!porColegio[colegioKey]) porColegio[colegioKey] = new Set();
        porColegio[colegioKey].add(personaKey);
    });

    const resumenPorColegio = Object.keys(porColegio).map(colegioKey => ({
        colegioKey,
        total: porColegio[colegioKey].size
    }));

    return {
        total: clavesGlobales.size,
        registrosValidos,
        descartadosSinNombre,
        porColegio: resumenPorColegio
    };
}

// Cuenta docente-talleres únicos.
// La clave es colegio + taller + nombre_normalizado.
// Un docente repetido dos veces en el mismo taller cuenta una sola vez.
// El mismo docente en varios talleres cuenta una vez por cada taller.
function contarParticipacionesUnicasPorTaller(encuestas, perfilObjetivo = 'Docente') {
    ...
    encuestas.forEach(d => {
        ...
        const colegioKey = normalizarColegioKey(d.colegio);
        const tallerKey = normalizarTallerKey(d);
        const personaKey = normalizarNombrePersona(d.asistente);

        if (!colegioKey || !tallerKey || !personaKey) {
            descartadosSinNombre++;
            return;
        }

        claves.add(`${colegioKey}||${tallerKey}||${personaKey}`);
    });
...
function contarDocentesUnicosPorColegioMap(encuestas) {
    const mapa = {};

    encuestas.forEach(d => {
        if (!perfilCoincide(d, 'Docente')) return;

        const colegioKey = normalizarColegioKey(d.colegio);
        const personaKey = normalizarNombrePersona(d.asistente);

        if (!colegioKey || !personaKey) return;

        if (!mapa[colegioKey]) mapa[colegioKey] = new Set();
        mapa[colegioKey].add(personaKey);
    });
...
async function initApp() {
    try {
        ...
        try {
            ...
            metaSnap.forEach(d => {
                if (d.id === 'cacheVersion') remoteVersion = String(d.data().updatedAt || '');
            });
        } catch (e) {
...
window.App = {
    switchTab: function(tabId) {
        ...
        ['btn-formTab', 'btn-dashTab', 'btn-coachTab'].forEach(btn => {
            const el = document.getElementById(btn);
            if (el) el.classList.remove('tab-active');
        });

        if (tabId === 'formTab') {
            const b = document.getElementById('btn-formTab');
            if (b) b.classList.add('tab-active');
        } else if (State.currentUserRole === 'admin') {
            const b = document.getElementById('btn-dashTab');
            if (b) b.classList.add('tab-active');
        } else {
            const b = document.getElementById('btn-coachTab');
            if (b) b.classList.add('tab-active');
        }
    ...
    initFiltrosBasicos: function() {
        ...
        if (dropLineas) {
            ...
            lineas.forEach(l => {
                dropLineas.innerHTML += `<label class="linea-label-container flex items-center space-x-2 p-2 hover:bg-orange-50 rounded-lg cursor-pointer transition-colors"><input type="checkbox" value="${escapeHtml(l)}" class="linea-cb accent-[#FF5A00] w-4 h-4" onchange="App.handleFilterTrigger()"><span class="text-xs font-medium text-gray-700">${escapeHtml(l)}</span></label>`;
            });
        }
    ...
    refreshCascadingFilters: function() {
        ...
        const updateSelect = (el, validData, text, currentVal) => {
            ...
            validData.forEach(v => el.innerHTML += `<option value="${escapeHtml(v)}">${escapeHtml(v)}</option>`);
        ...
        if (tSel && colEl) {
            if (colEl.value !== '') {
                tSel.disabled = false;
                tSel.classList.remove('cursor-not-allowed');
                document.getElementById('container_filter_taller')?.classList.remove('opacity-50');
                tSel.options[0].text = 'Todos los Talleres';
            } else {
                tSel.value = '';
                tSel.disabled = true;
                tSel.classList.add('cursor-not-allowed');
                document.getElementById('container_filter_taller')?.classList.add('opacity-50');
                tSel.options[0].text = 'Primero elija colegio';
            }
        }
    },
    ...
    updateDashboard: function() {
        ...
        try {
            ...
            const mesesF = mesNodes ? Array.from(mesNodes).map(cb => parseInt(cb.value, 10)) : [];
            ...
            const colegiosPermitidosGlobal = new Set(masterFilterGlobal.map(c => normalizarColegioKey(c.colegio)));

            let dataGlobal = safeEncuestas.map(d => {
                const colInfo = safeColegios.find(c => normalizarColegioKey(c.colegio) === normalizarColegioKey(d.colegio));
                return { ...d, regional: d.regional || (colInfo ? colInfo.regional : 'Sin Regional') };
            });

            dataGlobal = dataGlobal.filter(d => {
                if (!colegiosPermitidosGlobal.has(normalizarColegioKey(d.colegio))) return false;
            ...
            const computeAvance = (colegiosGrupo, encuestasGrupo) => {
                const colegiosCiclo = colegiosGrupo.filter(c => {
                    const clasif = c.clasificacion ? c.clasificacion.toUpperCase().trim() : '';
                    return !clasif.includes('PRODUCTO');
                });

                const nombresColegiosCiclo = new Set(colegiosCiclo.map(c => normalizarColegioKey(c.colegio)));

                let metaAsistencias = 0;
                colegiosCiclo.forEach(c => {
                    const docs = parseInt(c.docentes || c.Docentes || c.DOCENTES, 10) || 0;
                    const clasif = c.clasificacion ? c.clasificacion.toUpperCase().trim() : '';
                    const multiplicador = BusinessRules.metasCiclo[clasif] || 1;
                    metaAsistencias += docs * multiplicador;
                });

                const encuestasDeColegiosCiclo = encuestasGrupo.filter(d => nombresColegiosCiclo.has(normalizarColegioKey(d.colegio)));
                const participacionesInfo = contarParticipacionesUnicasPorTaller(encuestasDeColegiosCiclo, 'Docente');
                const asistenciasReales = participacionesInfo.total;
                ...
                return {
                    ...
                    asistenciasReales,
                    avance,
                    descartadosSinNombre: participacionesInfo.descartadosSinNombre
                };
            };

            const metaAlcance = masterFilter.reduce((s, c) => s + (parseInt(c.docentes || c.Docentes || c.DOCENTES, 10) || 0), 0);
            ...
            const avanceCiclo = metaAsistencias > 0 ? avanceGlobal.avance.toFixed(1) : '0.0';

            const docentesUnicosInfo = contarPersonasUnicasPorColegio(data, 'Docente');
            const directivosUnicosInfo = contarPersonasUnicasPorColegio(data, 'Directivo');
            const docs = docentesUnicosInfo.total;
            const dirs = directivosUnicosInfo.total;
            const encuestasAplicadas = data.length;
            const encuestasDocentes = data.filter(d => perfilCoincide(d, 'Docente')).length;
            const promedioTalleresPorDocente = docs > 0 ? (asistenciasReales / docs).toFixed(2) : '0.00';
            ...
            if (document.getElementById('dash_cobertura_colegios')) document.getElementById('dash_cobertura_colegios').innerText = new Set(data.map(d => normalizarColegioKey(d.colegio))).size;
            if (document.getElementById('dash_total_colegios')) document.getElementById('dash_total_colegios').innerText = masterFilter.length;

            // IDs opcionales. No rompen la app si no existen en el HTML.
            if (document.getElementById('dash_encuestas_aplicadas')) document.getElementById('dash_encuestas_aplicadas').innerText = encuestasAplicadas;
            if (document.getElementById('dash_encuestas_docentes')) document.getElementById('dash_encuestas_docentes').innerText = encuestasDocentes;
            if (document.getElementById('dash_promedio_talleres_docente')) document.getElementById('dash_promedio_talleres_docente').innerText = promedioTalleresPorDocente;
            if (document.getElementById('dash_registros_sin_nombre')) document.getElementById('dash_registros_sin_nombre').innerText = docentesUnicosInfo.descartadosSinNombre;

            // --- Asistencia promedio por regional ---
            // Ahora usa docentes únicos por colegio, no encuestas crudas.
            const asistenciaChartEl = document.getElementById('chart_regional_asistencia');
            if (asistenciaChartEl) {
                const docentesUnicosPorColegio = contarDocentesUnicosPorColegioMap(data);
                const porRegional = {};

                masterFilter.forEach(c => {
                    const key = normalizarColegioKey(c.colegio);
                    const encuestados = docentesUnicosPorColegio[key] ? docentesUnicosPorColegio[key].size : 0;
                    if (encuestados === 0) return;

                    const docentesAsignados = parseInt(c.docentes || c.Docentes || c.DOCENTES, 10) || 0;
            ...
            // --- Satisfacción promedio por regional ---
            ...
            if (regionalChartEl) {
                ...
                data.forEach(d => {
                    ...
                    if (count > 0) {
                        if (!porRegional[reg]) porRegional[reg] = { sum: 0, count: 0 };
                        porRegional[reg].sum += (sum / count);
                        porRegional[reg].count++;
                    }
                });

                const arrReg = Object.keys(porRegional).map(k => ({
                    name: k,
                    score: porRegional[k].sum / porRegional[k].count
                ...
                if (arrReg.length === 0) {
                    ...
                    regionalChartEl.innerHTML = arrReg.map(item => {
                        ...
                        return `<div><div class="flex justify-between text-xs font-bold mb-1.5 text-gray-600"><span>${escapeHtml(item.name)}</span><span style="color:${color}">${item.score.toFixed(1)}</span></div><div class="w-full bg-gray-100 rounded-full h-2.5"><div class="h-2.5 rounded-full transition-all duration-1000" style="width:${pct}%;background-color:${color}"></div></div></div>`;
                    }).join('');
                }
            }
            ...
            const buildRanking = (keyProp, dataset, colegiosDataset) => {
                ...
                const arr = Object.keys(rankMap).map(k => {
                    ...
                    const colegiosAsignados = new Set(colegiosGrupo.map(c => normalizarColegioKey(c.colegio)));
                    const colegiosEncuestados = new Set(dataset.filter(d => d[keyProp] === k).map(d => normalizarColegioKey(d.colegio)));

                    let encuestadosEnCartera = 0;
                    colegiosAsignados.forEach(c => {
                        if (colegiosEncuestados.has(c)) encuestadosEnCartera++;
                    });
                    ...
                    return {
                        name: k,
                        score: promedioReal,
                        count: rankMap[k].count,
                        cobertura,
                        colegiosEncuestados: encuestadosEnCartera,
                        colegiosAsignados: colegiosAsignados.size
                    };
    ...
    openBulkEditModal: function() {
        ...
        const safeCoaches = Array.isArray(State.coachesAuth) ? State.coachesAuth : [];
        [...new Set(safeCoaches.map(c => c.nombre))].sort().forEach(c => coachOptions += `<option value="${escapeHtml(c)}">${escapeHtml(c)}</option>`);

        let colOptions = '<option value="">(No modificar este campo)</option>';
        const safeColegios = Array.isArray(State.colegiosBD) ? State.colegiosBD : [];
        [...new Set(safeColegios.map(c => c.colegio))].sort().forEach(c => colOptions += `<option value="${escapeHtml(c)}">${escapeHtml(c)}</option>`);

        let regOptions = '<option value="">(No modificar este campo)</option>';
        [...new Set(safeColegios.map(c => c.regional).filter(Boolean))].sort().forEach(r => regOptions += `<option value="${escapeHtml(r)}">${escapeHtml(r)}</option>`);
    ...
    saveBulkEdit: async function() {
        ...
        if (!nuevoColegio && !nuevoCoach && !nuevoPerfil && !nuevaRegional) {
            alert('No seleccionaste ningún campo para modificar.');
            btn.disabled = false;
            btn.innerHTML = '<i class="fa-solid fa-bolt mr-2"></i>Aplicar Cambios';
            return;
        }
    ...
    populateRegistrosFilters: function() {
        ...
        const datosEnriquecidos = safeEncuestas.map(d => {
            const col = safeColegios.find(c => normalizarColegioKey(c.colegio) === normalizarColegioKey(d.colegio));
        ...
        const updateSelect = (el, validData, currVal, text) => {
            if (el) {
                el.innerHTML = `<option value="">${text}</option>`;
                validData.forEach(v => el.innerHTML += `<option value="${escapeHtml(v)}">${escapeHtml(v)}</option>`);
                if (validData.includes(currVal)) el.value = currVal;
                else el.value = '';
            }
        };
        ...
        if (State.currentUserRole === 'admin') {
            updateSelect(coachSel, [...new Set(getFilteredExcluding('coach').map(d => d.coach).filter(Boolean))].sort(), coachVal, 'Todos los Coaches');
        } else if (coachSel) {
            coachSel.innerHTML = `<option value="${escapeHtml(State.loginUser.nombre)}">${escapeHtml(State.loginUser.nombre)}</option>`;
            coachSel.disabled = true;
        }

        if (tallerSel) {
            const tallVal = tallerSel.value || '';
            updateSelect(tallerSel, [...new Set(getFilteredExcluding('taller').map(d => d.numTaller).filter(Boolean))].sort(), tallVal, 'Todos los Talleres');
        }
    },
    ...
    renderRegistrosTable: function(resetPagination = true) {
        ...
        if (fechaVal) {
            ...
            const selYear = parseInt(p[0], 10);
            const selMonth = parseInt(p[1], 10) - 1;
            const selDay = parseInt(p[2], 10);
    ...
    openEditRegistroModal: function(id) {
        ...
        const safeCoaches = Array.isArray(State.coachesAuth) ? State.coachesAuth : [];
        [...new Set(safeCoaches.map(c => c.nombre))].sort().forEach(c => coachOptions += `<option value="${escapeHtml(c)}" ${registro.coach === c ? 'selected' : ''}>${escapeHtml(c)}</option>`);

        let colOptions = '<option value="">Seleccione...</option>';
        [...new Set(safeColegios.map(c => c.colegio))].sort().forEach(c => colOptions += `<option value="${escapeHtml(c)}" ${registro.colegio === c ? 'selected' : ''}>${escapeHtml(c)}</option>`);

        const colEncontrado = safeColegios.find(c => normalizarColegioKey(c.colegio) === normalizarColegioKey(registro.colegio));
        ...
        [...new Set(safeColegios.map(c => c.regional).filter(Boolean))].sort().forEach(r => regOptions += `<option value="${escapeHtml(r)}" ${currentRegional === r ? 'selected' : ''}>${escapeHtml(r)}</option>`);
        ...
        this.showModal('Editar Registro', `
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-3 gap-4">
                    ...
                    <div><label class="text-xs font-bold text-gray-500">Asistente</label><input type="text" id="edit_reg_asistente" value="${escapeHtml(registro.asistente || '')}" class="w-full border rounded-xl px-3 py-2 text-sm"></div>
                </div>
    ...
    exportToExcel: function() {
        ...
        const data = list.map(d => {
            ...
            return {
                ...
                fecha: f,
                ciclo: d.ciclo,
                regional: d.regional || 'Sin Regional',
                colegio: d.colegio,
                asistente: d.asistente,
                asistente_normalizado: normalizarNombrePersona(d.asistente),
                perfil: d.perfil,
                coach: d.coach,
                numTaller: d.numTaller,
                taller: d.taller,
                q6: d.q6,
                q7: d.q7,
                q8: d.q8,
        ...
        XLSX.writeFile(wb, 'Reporte_Encuestas_Santillana.xlsx');
    },
    ...
    mostrarEstadoCobertura: function() {
        ...
        const colegiosConEncuestas = new Set(encuestasActuales.map(e => normalizarColegioKey(e.colegio)));

        const grouped = {};
        colegiosAsignados.forEach(c => {
            const reg = c.regional || 'Sin Regional';
            if (!grouped[reg]) grouped[reg] = [];
            const atendido = colegiosConEncuestas.has(normalizarColegioKey(c.colegio));
            grouped[reg].push({ nombre: c.colegio, coach: c.coach, atendido });
        });
            ...
            for (const [reg, colegios] of Object.entries(grouped)) {
                ...
                <div class="bg-gray-50 rounded-xl p-4 border border-gray-100">
                    <div class="flex justify-between items-center mb-3 border-b border-gray-200 pb-2">
                        <h4 class="font-black text-[#FF5A00] uppercase text-xs tracking-widest"><i class="fa-solid fa-map-location-dot mr-1"></i> ${escapeHtml(reg)}</h4>
                ...
                colegios.forEach(c => {
                    ...
                    <div class="flex flex-col p-2.5 rounded-lg border ${bgStyle}">
                        <div class="flex items-center gap-2">
                            ${icon}
                            <span class="text-xs font-bold ${textStyle} truncate" title="${escapeHtml(c.nombre)}">${escapeHtml(c.nombre)}</span>
                        </div>
                        ${State.currentUserRole === 'admin' && !coachF ? `<span class="text-[9px] text-gray-500 ml-6 mt-0.5"><i class="fa-solid fa-user-tie opacity-50 mr-1"></i>${escapeHtml(c.coach)}</span>` : ''}
                    </div>`;
                });
    ...
    submitForm: async function(e) {
        ...
        if (!safeColegios.some(c => c.colegio === inputColegio)) {
            alert('⚠️ Por favor, selecciona un colegio válido de la lista desplegable.');
            return;
        }
        ...
        const formData = {
            ...
            asistente: document.getElementById('q2_asistente').value,
            perfil: document.querySelector('input[name="q3_perfil"]:checked').value,
            coach: document.getElementById('q4_coach').value,
            numTaller: document.getElementById('q5_numTaller').value,
            taller: document.getElementById('q5_taller').value,
            q6: parseInt(document.querySelector('input[name="q6"]:checked').value, 10),
            q7: parseInt(document.querySelector('input[name="q7"]:checked').value, 10),
            q8: parseInt(document.querySelector('input[name="q8"]:checked').value, 10),
            q9: parseInt(document.querySelector('input[name="q9"]:checked').value, 10),
        ...
        try {
            ...
            alert('Hubo un error guardando la encuesta. Intenta de nuevo.');
        } finally {
            btn.disabled = false;
            btn.innerHTML = '<span>Enviar Feedback</span>';
        }
    },

    handleColegioInput: function() {
        const val = document.getElementById('q1_colegio').value.toLowerCase();
        const list = document.getElementById('listaColegiosCustom');
        list.innerHTML = '';
        ...
        if (matches.length > 0) {
            matches.forEach(m => {
                ...
                li.className = 'px-4 py-3 hover:bg-orange-50 cursor-pointer text-sm border-b flex justify-between';
                li.innerHTML = `<span>${escapeHtml(m.colegio)}</span> <span class="text-[10px] text-gray-400 font-bold">${escapeHtml(m.regional)}</span>`;
                li.onclick = () => {
                    document.getElementById('q1_colegio').value = m.colegio;
                    document.getElementById('q4_coach').value = m.coach;
                    list.classList.add('hidden');
                };
                list.appendChild(li);
            });
    ...
    poblarCoaches: function() {
        ...
        if (sel) {
            ...
            const safeColegios = Array.isArray(State.colegiosBD) ? State.colegiosBD : [];
            [...new Set(safeColegios.map(c => c.coach).filter(Boolean))].sort().forEach(c => sel.innerHTML += `<option value="${escapeHtml(c)}">${escapeHtml(c)}</option>`);
        }
    },

    openImportModal: function() {
        this.showModal('Importar', `<div class="flex gap-4 pt-2"><button type="button" onclick="App.downloadEncuestaTemplate()" class="flex-1 py-3 bg-gray-100 rounded-xl">Plantilla</button><button type="button" onclick="document.getElementById('fileImportEncuestas').click()" class="flex-1 py-3 bg-[#002C5F] text-white rounded-xl">Subir Excel</button></div>`, `<button type="button" onclick="App.hideModal()" class="px-6 py-2 bg-gray-200 rounded-xl">Cerrar</button>`);
    },

    downloadEncuestaTemplate: function() {
        const ws = XLSX.utils.json_to_sheet([{
            id: 'DEJAR_VACIO_NUEVO_REGISTRO',
            fecha: new Date().toLocaleDateString('es-CO'),
            ciclo: 'Ciclo Inclusión',
            ...
            q8: 5,
            q9: 5,
            sugerencias: 'Buen taller.'
        }]);

        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, 'Encuestas');
        XLSX.writeFile(wb, 'Plantilla_Encuestas.xlsx');
    },

    clearData: async function() {
        if (!confirm('⚠️ PELIGRO: ¿Borrar todas las encuestas en la nube?')) return;

        try {
            const btn = document.getElementById('btn_clear_data');
            btn.innerText = 'Borrando...';
            btn.disabled = true;

            const safeEncuestas = Array.isArray(State.encuestasData) ? State.encuestasData : [];
            for (const enc of safeEncuestas) await deleteDoc(doc(db, 'encuestas', enc.id));
            ...
            document.getElementById('btn_clear_data').innerHTML = 'Borrar Todo';
            document.getElementById('btn_clear_data').disabled = false;
        }
    },
    ...
    switchDashSubTab: function(id) {
        ['subTabResultados', 'subTabRegistros', 'subTabEdicion'].forEach(t => {
        ...
        ['btn-sub-resultados', 'btn-sub-registros', 'btn-sub-edicion'].forEach(b => {
    ...
    handleEdicionFilterTrigger: function() {
        this.refreshEdicionFilters();
        this.renderActiveEdicionTable();
    },

    refreshEdicionFilters: function() {
        const safeColegios = Array.isArray(State.colegiosBD) ? State.colegiosBD : [];
        const base = safeColegios.filter(c => c.colegio !== '[NUEVO COACH SIN COLEGIO]');
        const regEl = document.getElementById('edicion_filter_regional');
        const coachEl = document.getElementById('edicion_filter_coach');

        const updateDropdown = (el, validOptions, defaultText, currentVal) => {
            if (!el) return;
            el.innerHTML = `<option value="">${defaultText}</option>`;
            validOptions.forEach(v => el.innerHTML += `<option value="${escapeHtml(v)}">${escapeHtml(v)}</option>`);
            if (validOptions.includes(currentVal)) el.value = currentVal;
            else el.value = '';
        };
    ...
    renderActiveEdicionTable: function() {
        const head = document.getElementById('edicion_head');
        const body = document.getElementById('edicion_body');
        if (!head || !body) return;
        ...
        if (State.currentEdicionView === 'colegios') {
            ...
            head.innerHTML = `<tr><th class="px-6 py-4">Colegio</th><th class="px-6 py-4">Regional</th><th class="px-6 py-4">Coach</th><th class="px-6 py-4 text-center">Acciones</th></tr>`;
            body.innerHTML = '';
            ...
            safeColegios.forEach((c, idx) => {
                if ((searchVal === '' || (c.colegio || '').toLowerCase().includes(searchVal)) && (regVal === '' || c.regional === regVal) && (coachVal === '' || c.coach === coachVal)) {
                    body.innerHTML += `<tr class="hover:bg-gray-50 border-b"><td class="px-6 py-4 font-bold">${escapeHtml(c.colegio)}</td><td class="px-6 py-4">${escapeHtml(c.regional)}</td><td class="px-6 py-4">${escapeHtml(c.coach)}</td><td class="px-6 py-4 text-center"><button type="button" onclick="App.openEditColegioModal(${idx})" class="text-blue-500 mx-1"><i class="fa-solid fa-pen"></i></button><button type="button" onclick="App.deleteColegio(${idx})" class="text-red-500 mx-1"><i class="fa-solid fa-trash"></i></button></td></tr>`;
                }
            });
        } else {
            head.innerHTML = `<tr><th class="px-6 py-4">Coach</th><th class="px-6 py-4">Email</th><th class="px-6 py-4">Regional base</th><th class="px-6 py-4 text-center">Acciones</th></tr>`;
            body.innerHTML = '';

            const safeCoaches = Array.isArray(State.coachesAuth) ? State.coachesAuth : [];
            safeCoaches.forEach((c, idx) => {
                if (searchVal === '' || (c.nombre || '').toLowerCase().includes(searchVal) || (c.email || '').toLowerCase().includes(searchVal)) {
                    body.innerHTML += `<tr class="hover:bg-gray-50 border-b"><td class="px-6 py-4 font-bold">${escapeHtml(c.nombre)}</td><td class="px-6 py-4">${escapeHtml(c.email)}</td><td class="px-6 py-4">${escapeHtml(c.regional)}</td><td class="px-6 py-4 text-center"><button type="button" onclick="App.openEditCoachModal(${idx})" class="text-blue-500 mx-1"><i class="fa-solid fa-pen"></i></button><button type="button" onclick="App.deleteCoach(${idx})" class="text-red-500 mx-1"><i class="fa-solid fa-trash"></i></button></td></tr>`;
                }
            });
        }
    },

    deleteColegio: async function(idx) {
        if (!confirm('¿Borrar colegio?')) return;
        try {
            await deleteDoc(doc(db, 'colegios', State.colegiosBD[idx].id));
            State.colegiosBD.splice(idx, 1);
    ...
    deleteCoach: async function(idx) {
        if (!confirm('¿Borrar coach?')) return;
        try {
            await deleteDoc(doc(db, 'coaches', State.coachesAuth[idx].id));
            State.coachesAuth.splice(idx, 1);
            await invalidateRemoteCache();
            this.renderActiveEdicionTable();
        } catch (e) {
            alert('Error');
    ...
    openEditColegioModal: function(idx) {
        const isEdit = idx >= 0;
        const data = isEdit ? State.colegiosBD[idx] : { colegio: '', regional: '', coach: '', docentes: 0, calendario: 'A', lineaNegocio: 'Compartir', clasificacion: 'AA' };
        const safeColegios = Array.isArray(State.colegiosBD) ? State.colegiosBD : [];

        let regOptions = '<option value="">Seleccione Regional...</option>';
        [...new Set(safeColegios.map(c => c.regional).filter(Boolean))].sort().forEach(r => regOptions += `<option value="${escapeHtml(r)}" ${data.regional === r ? 'selected' : ''}>${escapeHtml(r)}</option>`);

        this.showModal(isEdit ? 'Editar Institución' : 'Nuevo Colegio', `<div class="space-y-4"><div><label class="text-xs font-bold">Nombre</label><input type="text" id="edit_col_nombre" value="${escapeHtml(data.colegio)}" class="w-full border rounded-xl px-4 py-2"></div><div class="grid grid-cols-2 gap-4"><div><label class="text-xs font-bold">Regional</label><select id="edit_col_reg" class="w-full border rounded-xl px-4 py-2">${regOptions}</select><input type="text" id="edit_col_reg_nueva" placeholder="O escriba una regional nueva..." class="w-full border rounded-xl px-4 py-2 mt-2 text-xs"></div><div><label class="text-xs font-bold">Coach</label><select id="edit_col_coach" class="w-full border rounded-xl px-4 py-2"><option value="${escapeHtml(data.coach || '')}">${escapeHtml(data.coach || 'Seleccione...')}</option></select></div></div><div class="grid grid-cols-2 gap-4 border-t pt-4"><div><label class="text-[10px] font-bold">Docentes</label><input type="number" id="edit_col_docentes" value="${data.docentes || 0}" class="w-full border rounded-xl px-4 py-2"></div><div><label class="text-[10px] font-bold">Clasificación</label><input type="text" id="edit_col_clasif" value="${escapeHtml(data.clasificacion || '')}" class="w-full border rounded-xl px-4 py-2"></div></div></div>`, `<button type="button" onclick="App.hideModal()" class="px-6 py-2 bg-gray-200 rounded-xl">Cancelar</button><button type="button" onclick="App.saveEditColegio(${idx})" class="px-6 py-2 bg-[#FF5A00] text-white rounded-xl">Guardar</button>`);
    ...
    openEditCoachModal: function(idx) {
        ...
        this.showModal('Editar Coach', `<div class="space-y-4"><div><label class="text-xs font-bold">Nombre</label><input type="text" id="edit_coach_nombre" value="${escapeHtml(data.nombre)}" class="w-full border rounded-xl px-4 py-2"></div><div><label class="text-xs font-bold">Email</label><input type="email" id="edit_coach_email" value="${escapeHtml(data.email)}" class="w-full border rounded-xl px-4 py-2"></div><div class="grid grid-cols-2 gap-4"><div><label class="text-xs font-bold">Regional base</label><input type="text" id="edit_coach_regional" value="${escapeHtml(data.regional)}" class="w-full border rounded-xl px-4 py-2"><p class="text-[10px] text-gray-400 mt-1">Solo informativo. La zona real la define cada colegio asignado al coach, que puede pertenecer a varias regionales.</p></div><div><label class="text-xs font-bold">Contraseña</label><input type="text" id="edit_coach_pass" value="${escapeHtml(data.pass || '')}" class="w-full border rounded-xl px-4 py-2"></div></div></div>`, `<button type="button" onclick="App.hideModal()" class="px-6 py-2 bg-gray-200 rounded-xl">Cancelar</button><button type="button" onclick="App.saveEditCoach(${idx})" class="px-6 py-2 bg-[#002C5F] text-white rounded-xl">Guardar</button>`);
    },
    ...
    updateCoachOptionsEdit: function(selectedCoach = '') {
        ...
        const safeCoaches = Array.isArray(State.coachesAuth) ? State.coachesAuth : [];
        [...safeCoaches].sort((a, b) => (a.nombre || '').localeCompare(b.nombre || '')).forEach(c => selCoach.innerHTML += `<option value="${escapeHtml(c.nombre)}" ${c.nombre === selectedCoach ? 'selected' : ''}>${escapeHtml(c.nombre)}</option>`);
    },
    ...
    saveEditColegio: async function(idx) {
        ...
        const colData = {
            colegio: document.getElementById('edit_col_nombre').value.trim().toUpperCase(),
            regional,
            coach: document.getElementById('edit_col_coach').value,
            docentes: parseInt(document.getElementById('edit_col_docentes').value, 10) || 0,
            clasificacion: document.getElementById('edit_col_clasif').value.trim()
        };

        if (!colData.colegio || !colData.regional || !colData.coach) {
            alert('Completa Nombre, Regional y Coach.');
            return;
        }

        try {
            if (idx === -1) {
                const docRef = await addDoc(collection(db, 'colegios'), colData);
                State.colegiosBD.unshift({ id: docRef.id, ...colData });
            } else {
                const colId = State.colegiosBD[idx].id;
                await setDoc(doc(db, 'colegios', colId), colData);
                State.colegiosBD[idx] = { id: colId, ...colData };
            }
    ...
    saveEditCoach: async function(idx) {
        const coachData = {
            nombre: document.getElementById('edit_coach_nombre').value.trim(),
            email: document.getElementById('edit_coach_email').value.trim().toLowerCase(),
            regional: document.getElementById('edit_coach_regional').value.trim(),
            pass: document.getElementById('edit_coach_pass').value.trim()
        };

        if (!coachData.nombre || !coachData.email || !coachData.regional) {
            alert('Faltan datos.');
            return;
        }

        try {
            const coachId = State.coachesAuth[idx].id;
            await setDoc(doc(db, 'coaches', coachId), coachData);
            State.coachesAuth[idx] = { id: coachId, ...coachData };
            await invalidateRemoteCache();
            this.hideModal();
            this.renderActiveEdicionTable();
            this.poblarCoaches();
    ...
    exportData: function(type) {
        const data = type === 'colegios' ? State.colegiosBD : State.coachesAuth;
        const sheetName = type === 'colegios' ? 'Colegios_BD' : 'Coaches_Auth';
        const ws = XLSX.utils.json_to_sheet(data);
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, sheetName);
        XLSX.writeFile(wb, `BaseDatos_${type}.xlsx`);
    }
};

function initRatings() {
    ['q6', 'q7', 'q8', 'q9'].forEach(q => {
        const container = document.getElementById(`${q}_group`);
        if (container) {
            for (let i = 1; i <= 5; i++) {
                container.innerHTML += `<label class="rating-label"><input type="radio" name="${q}" value="${i}" required><span class="text-xs mt-1">${i}</span></label>`;
            }
        }
    });
}
    ...
    reader.onload = async evt => {
        ...
        try {
            ...
            for (const col of data) {
                ...
                const safeColegios = Array.isArray(State.colegiosBD) ? State.colegiosBD : [];
                const existingIdx = safeColegios.findIndex(c => normalizarColegioKey(c.colegio) === normalizarColegioKey(cleanCol.colegio));

                if (existingIdx >= 0) {
                    const docId = State.colegiosBD[existingIdx].id;
                    await setDoc(doc(db, 'colegios', docId), cleanCol);
                    State.colegiosBD[existingIdx] = { id: docId, ...cleanCol };
                } else {
                    const docRef = await addDoc(collection(db, 'colegios'), cleanCol);
                    State.colegiosBD.push({ id: docRef.id, ...cleanCol });
                }
            }

            await invalidateRemoteCache();
            App.hideModal();
            if (State.currentEdicionView === 'colegios') App.renderActiveEdicionTable();
...
document.getElementById('fileImportCoaches')?.addEventListener('change', e => {
    ...
    reader.onload = async evt => {
        ...
        try {
            const wb = XLSX.read(evt.target.result, { type: 'binary' });
            const importedCoaches = XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]]);
            ...
            for (const newCoach of importedCoaches) {
                ...
                if (coachExistenteIdx >= 0) {
                    const docId = State.coachesAuth[coachExistenteIdx].id;
                    const passActual = State.coachesAuth[coachExistenteIdx].pass;
                    const updatedData = { ...newCoach, pass: passActual };
                    delete updatedData.id;
                    await setDoc(doc(db, 'coaches', docId), updatedData);
                    State.coachesAuth[coachExistenteIdx] = { id: docId, ...updatedData };
                } else {
                    const cleanCoach = { ...newCoach };
                    if (!cleanCoach.pass) cleanCoach.pass = '12345';
                    delete cleanCoach.id;
                    const docRef = await addDoc(collection(db, 'coaches'), cleanCoach);
                    State.coachesAuth.push({ id: docRef.id, ...cleanCoach });
                }
            }

            await invalidateRemoteCache();
            App.hideModal();
            if (State.currentEdicionView === 'coaches') App.renderActiveEdicionTable();
            alert('Coaches actualizados.');
        } catch (err) {
            App.hideModal();
            alert('Error');
        }
        ...
        try {
            ...
            for (const row of importedData) {
                ...
                if (row.id && row.id !== 'DEJAR_VACIO_NUEVO_REGISTRO') {
                    ...
                    if (existingIdx >= 0) {
                        await setDoc(doc(db, 'encuestas', row.id.toString()), row);
                        State.encuestasData[existingIdx] = { ...State.encuestasData[existingIdx], ...row, id: row.id.toString() };
                    } else {
                        await setDoc(doc(db, 'encuestas', row.id.toString()), row);
                        State.encuestasData.push({ ...row, id: row.id.toString() });
                    }
                } else {
                    const cleanRow = { ...row };
                    if (!cleanRow.timestamp) cleanRow.timestamp = Date.now();
                    delete cleanRow.id;
                    const docRef = await addDoc(collection(db, 'encuestas'), cleanRow);
                    State.encuestasData.push({ id: docRef.id, ...cleanRow });
                }
            }

            App.hideModal();
            App.handleFilterTrigger();
...
window.onload = async () => {
    ...
    window.addEventListener('click', function(e) {
        const dropColegios = document.getElementById('listaColegiosCustom');
        const inputColegio = document.getElementById('q1_colegio');
        if (dropColegios && inputColegio && !dropColegios.contains(e.target) && e.target !== inputColegio) dropColegios.classList.add('hidden');

        const dropLineas = document.getElementById('dropdown_lineas');
        const btnLineas = document.getElementById('btn_dropdown_lineas');
        if (dropLineas && btnLineas && !dropLineas.contains(e.target) && !btnLineas.contains(e.target)) {
            dropLineas.classList.add('hidden');
            dropLineas.setAttribute('aria-hidden', 'true');
            if (btnLineas) btnLineas.setAttribute('aria-expanded', 'false');
        }

        const dropMeses = document.getElementById('dropdown_meses');
        const btnMeses = document.getElementById('btn_dropdown_meses');
        if (dropMeses && btnMeses && !dropMeses.contains(e.target) && !btnMeses.contains(e.target)) {
            dropMeses.classList.add('hidden');
            dropMeses.setAttribute('aria-hidden', 'true');
            if (btnMeses) btnMeses.setAttribute('aria-expanded', 'false');
        }
    });
};
